...
{
    public class BusinessApplication : ComponentBase
    {
        ...
        private List<BusinessApplication> _ComponentBusinessApplications = new List<BusinessApplication>();
        ...
        /// </summary>
        public List<BusinessApplication> ComponentBusinessApplications
        {
            get
            {
                return _ComponentBusinessApplications;
            }
        }
        ...
        public List<BusinessApplication> MandatoryBusinessApplications
        {
            get
            {
                List<BusinessApplication> Result = new List<BusinessApplication>();

                foreach (BusinessApplication Service in _ComponentBusinessApplications)
                {
        ...
        ///// </summary>
        //public List<BusinessApplication> BusinessApplicationsUsedBy
        //{
        //    get
        //    {
        //        return _BusinessApplicationsUsedBy;
        //    }
        //}
        ...
        public BusinessApplication(string Name, Global.HealthState State)
            : base()
        {
            base.Name = Name;
            //base.State = State;
        ...
        public void AddRelationship(Relationship Relationship)
        {
            ...
            if (Global.GetExistingRelationship(Relationship, _Relationships) == null)
            {
                ...
                {
                    case Dependency.Relationship.ComponentType.BusinessApplication:

                        // Check if the Service exists
                        if (Global.GetExistingBusinessApplication(Relationship.ToBusinessApplication, _ComponentBusinessApplications) == null)
                        {
                            // Add the Component Service
                            _ComponentBusinessApplications.Add(Relationship.ToBusinessApplication);
                        }
        ...
        public void CalculateState(string Reason)
        {
            ...
            if (!_StateSetAtServiceLevel) // <- to allow emulation or to ignore component health state
            {
                ...
                if (this.MandatoryBusinessApplications.Count == 0 && this.MandatoryServers.Count == 0)      // No components
                {
                    this.SetSystemHealthState(Global.HealthState.OK, "This Business Application has no Components");
                }
                ...
                else if (this.MandatoryServers.Count == 0 && this.MandatoryBusinessApplications.Count > 0)  // Has Services only
                {
                    // System State
                    BusinessApplication ComponentService = this.MandatoryBusinessApplications.OrderByDescending(s => s.SystemState)
                                                                                     .FirstOrDefault();
                    ...
                    // User State
                    ComponentService = this.MandatoryBusinessApplications.OrderByDescending(s => s.UserState)
                                                                     .FirstOrDefault();
                ...
                else if (this.MandatoryBusinessApplications.Count == 0 && this.MandatoryServers.Count > 0) // Has Servers only
                {
                    // System State
                ...
                else if (this.MandatoryBusinessApplications.Count > 0 && this.MandatoryServers.Count > 0)   // Has Servers and Services
                {
                    #region Service

                    // Service System State
                    BusinessApplication WorstSystemStateService = _ComponentBusinessApplications.OrderByDescending(s => s.SystemState)
                                                                                  .FirstOrDefault();

                    // Service User State
                    BusinessApplication WorstUserStateService = this.MandatoryBusinessApplications.OrderByDescending(s => s.UserState)
                                                                                      .FirstOrDefault();
        ...
        public Relationship GetRelationship(BusinessApplication Service)
        {
            return _Relationships.Find(x => Service == x.ToBusinessApplication);
        }
        ...
        public void Save()
        {
            ...
            // Service Comment 1
            //Query = "UPDATE AttributeValueText SET AttributeValue = '" + _Comment1.Replace(SingleQuote, TwoSingleQuotes) + "' WHERE ObjectId = '" + base.ToString() + "' AND AttributeId = 'D61B7818-63E2-4533-8940-86C206181175' AND VersionId = '" + base.VersionID.ToString() + "'";
            //_SQL.ExecuteNonQuery(Query);
        ...
        public override void RaiseNewSystemState(Global.HealthState NewState, Global.HealthState OldState, string Name, string Text)
        {
            if (NewState != OldState)
            {
                ...
                {
                    Debug.WriteLine("[INF1120] " + base.Name + " (Business Application) has changed state from " + OldState + " to " + NewState, "debug");
                }
            }
        }
